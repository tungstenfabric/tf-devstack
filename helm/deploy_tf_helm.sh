...
if [ "$ORCHESTRATOR" == "kubernetes" ]; then
  ...
  helm upgrade neutron $WORKSPACE/openstack-helm/neutron --namespace=openstack --recreate-pods --force \
    --values $WORKSPACE/openstack-helm/neutron/values_overrides/tf.yaml \
    --set images.tags.tf_neutron_init=$CONTAINER_REGISTRY/contrail-openstack-neutron-init:${CONTRAIL_CONTAINER_TAG}
  helm upgrade nova $WORKSPACE/openstack-helm/nova --namespace=openstack --recreate-pods --force \
    --values $WORKSPACE/openstack-helm/nova/values_overrides/tf.yaml \
