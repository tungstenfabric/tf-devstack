...
if [ "$ORCHESTRATOR" == "kubernetes" ]; then
  ...
  helm upgrade neutron $WORKSPACE/openstack-helm/neutron --namespace=openstack --force \
  ...
  helm upgrade nova $WORKSPACE/openstack-helm/nova --namespace=openstack --force \
