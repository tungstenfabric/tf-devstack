...
function machines() {
    ...
    sudo python3 -m pip uninstall -y requests docker-compose urllib3 chardet docker docker-py
    ...
    sudo python3 -m pip install 'docker-compose==1.24.1' 'ansible==2.7.11' jinja2 requests
    ...
    python3 $(which ansible) -m "copy" --args="content=c dest='/tmp/rekjreekrbjrekj.txt'" localhost
    rm -rf /tmp/rekjreekrbjrekj.txt

    # Keep default python due to ansible modules bindings (packages module requires py2 rpm binding)
...
function k8s() {
    if [[ "$ORCHESTRATOR" != "kubernetes" ]]; then
        ...
        sudo -E python3 $(which ansible-playbook) -v -e orchestrator=$ORCHESTRATOR \
            -e config_file=$tf_deployer_dir/instances.yaml \
