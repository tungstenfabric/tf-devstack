...
function wait_vhost0_up() {
    ...
    for node in ${AGENT_NODES} ; do
        scp $SSH_OPTIONS ${fmy_dir}/functions.sh ${SSH_USER}@${node}:/tmp/functions.sh
        if ! ssh $SSH_OPTIONS ${SSH_USER}@${node} "export PATH=\$PATH:/usr/sbin ; source /tmp/functions.sh ; wait_nic_up vhost0" ; then
            return 1
        fi
    done
}
