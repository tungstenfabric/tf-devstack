...
sudo virsh destroy ${KUBERNETES_CLUSTER_NAME}-bootstrap || echo "INFO: virsh destroy ${KUBERNETES_CLUSTER_NAME}-bootstrap failed"
sudo virsh undefine ${KUBERNETES_CLUSTER_NAME}-bootstrap --remove-all-storage || echo "INFO: virsh undefine ${KUBERNETES_CLUSTER_NAME}-bootstrap --remove-all-storage failed"
