...
function setup_keystone_auth(){
  ...
  if [[ -z $keystone_address ]] ; then
    echo "Cannot detect the keystone address. It is needed for reachabilty to keystone from keystone-auth-pods."
    echo ""
    exit 1
  fi
  ...
  if [[ -z $host_address ]] ; then
    echo "Cannot detect the host address for machine with keystone. It is needed for reachabilty to keystone from keystone-auth-pods."
    echo ""
    exit 1
  fi
