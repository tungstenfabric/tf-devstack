...
function check_kubernetes_master_cert() {
  ...
  if sudo grep -q "IP Address:10.96.0.1" /root/cdk/server.crt ; then
    # only if cert doesn't have specific server's IP
    return 0
  fi

  # change setting
  command juju config kubernetes-master extra_sans="fake-name-$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 4 | head -n 1)"
