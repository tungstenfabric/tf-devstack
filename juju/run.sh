...
function collect_deployment_env() {
    ...
    if [[ "${SSL_ENABLE,,}" == 'true' ]] ; then
        ...
        DEPLOYMENT_ENV['SSL_KEY']="$(sudo find /etc/contrail 2>/dev/null | grep server-privkey.pem | head -1 | xargs sudo base64)"
        DEPLOYMENT_ENV['SSL_CERT']="$(sudo find /etc/contrail 2>/dev/null | grep server.pem | head -1 | xargs sudo base64)"
        DEPLOYMENT_ENV['SSL_CACERT']="$(sudo find /etc/contrail 2>/dev/null | grep ca-cert.pem | head -1 | xargs sudo base64)"
    fi
}
