...
function workaround_kubesray_docker_cli() {
  ...
  declare -A iparr=( ${CONTROLLER_NODES} ${AGENT_NODES} )
  ...
  if [[ "$DISTRO" == "centos" || "$DISTRO" == "rhel" ]]; then
    ...
    do
      ...
      echo $userlink
      echo ""
      ssh $userlink sudo yum install -y yum-utils yum-plugin-versionlock && sudo yum-config-manager --add-repo ${DOCKER_REPO} && sudo yum-config-manager --add-repo ${DOCKER_REPO} && sudo yum versionlock status
      echo ""
    done
