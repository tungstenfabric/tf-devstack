...
function workaround_kubespray_docker_cli() {
  ...
  if [[ "$DISTRO" == "centos" || "$DISTRO" == "rhel" ]]; then
    ...
    for node in $nodes; do
      ...
      echo $userlink
      ssh -t $userlink "sudo yum install -y yum-utils yum-plugin-versionlock; sudo yum-config-manager --add-repo $DOCKER_REPO; sudo yum versionlock $DOCKER_CLI_VERSION; sudo yum versionlock status"
    done
