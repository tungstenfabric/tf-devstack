...
# Copy devstack-directory to another nodes to be able to run create_docker_config.sh there
...
for machine in $(echo "$CONTROLLER_NODES $AGENT_NODES" | tr " " "\n" | sort -u); do
    if ! ip a | grep -q "$machine"; then
        ...
        scp -r $SSH_OPTIONS $(dirname $my_dir) $machine:/tmp/
    fi
done
...
cd kubespray/
# deploy kubespray in virtualenv to avoid package conflicts late
sudo pip3 install virtualenv
virtualenv .venv
source .venv/bin/activate
pip3 install -chttps://releases.openstack.org/constraints/upper/master
...
if ! [ -e inventory/mycluster/hosts.yml ] && [[ "$LOOKUP_NODE_HOSTNAMES" == "true" ]]; then
    ...
    for ip in $(echo ${IPS[@]} | tr ' ' '\n' | awk '!x[$0]++'); do
        ...
        hostname=$(ssh $SSH_OPTIONS $ip hostname -s)
        IPS_WITH_HOSTNAMES[$hostname]=$ip
       ((node_count+=1))
    done
    # Test if all hostnames were unique
    if [[ "${#IPS_WITH_HOSTNAMES[@]}" != "$node_count" ]]; then
        echo "ERROR: Not all hosts have unique hostnames."
        echo "ERROR: To use automatic host naming, set LOOKUP_NODE_HOSTNAMES=false"
        exit 1
    fi
...
for machine in $(echo "$CONTROLLER_NODES $AGENT_NODES" | tr " " "\n" | sort -u); do
    if ! ip a | grep -q "$machine"; then
        ssh $SSH_OPTIONS $machine "sudo yum install -y python3 python3-pip"
        ssh $SSH_OPTIONS $machine "export CONTAINER_REGISTRY=$CONTAINER_REGISTRY ; sudo -E /tmp/${devstack_dir}/common/create_docker_config.sh"
    fi
done
...
# exit from virtualenv - kubespray has been deployed
deactivate
...
if [[ "openstack" == "${ORCHESTRATOR}" && "${CNI}" == "calico" ]] ; then
    ...
    echo "INFO: Patch calico CNI template on all nodes"
    for machine in $(echo "$CONTROLLER_NODES $AGENT_NODES" | tr " " "\n" | sort -u); do
        echo "INFO: Patch calico CNI template at $machine"
        ssh $SSH_OPTIONS $machine <<'PATCH'
