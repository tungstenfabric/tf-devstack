...
for machine in $(echo "$CONTROLLER_NODES $AGENT_NODES" | tr " " "\n" | sort -u); do
  if ! ip a | grep -q "$machine"; then
    ssh $ssh_opts $machine "mkdir -p ./files"
    ssh $ssh_opts $machine "sudo cp /tmp/tf-devstack/common/files/docker_daemon.json.j2 files/"
    ssh $ssh_opts $machine "sudo -E /tmp/tf-devstack/common/create_docker_config.sh"
  fi
done
