...
#    fi
    ...
    if [[ -n "$lxd_url" ]]; then
       ...
       ssh $SSH_USER@$machine "lxc image import bionic-server-cloudimg-amd64-lxd.tar.xz bionic-server-cloudimg-amd64-root.tar.xz --alias juju/bionic/amd64 > /dev/null && lxc image list"
