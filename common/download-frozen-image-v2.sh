...
fetch_blob() {
    ...
    local curlString="curl -sS $auth_header $registryBase/v2/$image/blobs/$digest -o $targetFile -D-"
    ...
    if grep -qE "^HTTP/[0-9].[0-9] 3" <<< "$curlHeaders"; then
        ...
        curl -fsSL "$blobRedirect" -o "$targetFile"
    fi
}
...
handle_single_manifest_v2() {
    ...
    echo "INFO: Downloading '$imageIdentifier' (${#layers[@]} layers)..."
    local layerId=
    local layerFiles=()
    for i in "${!layers[@]}"; do
        ...
        case "$layerMediaType" in
            application/vnd.docker.image.rootfs.diff.tar.gzip)
                ...
                if [ -f "$dir/$layerTar" ]; then
                    ...
                    echo "INFO: skipping existing ${layerId:0:12}"
                    continue
                fi
                ...
                fetch_blob "$token" "${namespace}${image}" "$layerDigest" "$dir/$layerTar"
                ;;
...
echo "INFO: Download of images into '$dir' complete."
echo "INFO: Use something like the following to load the result into a Docker daemon:"
echo "INFO:   tar -cC '$dir' . | docker load"
