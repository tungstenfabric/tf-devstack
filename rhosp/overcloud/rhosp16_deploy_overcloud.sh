...
if [[ "$USE_PREDEPLOYED_NODES" == false ]]; then
   ...
   if [[ -z "${overcloud_compute_prov_ip}" ]]; then
    ...
    if [[ $OPENSTACK_VERSION == 'train' ]] ; then
      python3 ~/tripleo-heat-templates/tools/process-templates.py -p ~/tripleo-heat-templates/environments/ \
       -r ~/tripleo-heat-templates/roles_data.yaml -n  ~/tripleo-heat-templates/network_data.yaml
      openstack overcloud deploy --templates tripleo-heat-templates/ \
         --stack overcloud --libvirt-type kvm \
         --roles-file tripleo-heat-templates/roles_data_contrail_aio.yaml \
         --disable-validations \
