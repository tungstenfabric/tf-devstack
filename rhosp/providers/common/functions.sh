...
function collect_overcloud_env() {
    ...
    if [[ -f ~/overcloudrc ]] ; then
        ...
        source ~/overcloudrc
        internal_vip=$(ssh $ssh_opts $SSH_USER@$openstack_node sudo hiera -c /etc/puppet/hiera.yaml internal_api_virtual_ip)
        os_auth_internal_api_url=$(echo $OS_AUTH_URL | sed "s#[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*\+#$internal_vip#")
