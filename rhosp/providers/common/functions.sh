...
function update_undercloud_etc_hosts() {
    ...
    if [[ "$USE_PREDEPLOYED_NODES" == true ]]; then
        ...
        local public_vip=$(echo $overcloud_cont_prov_ip | cut -d ',' -f 1)
        local internal_api_vip=$public_vip
        local ctlplane_vip=$public_vip
    else
...
function collect_overcloud_env() {
    ...
    if [[ -z "$overcloud_ctrlcont_instance" && -z "$overcloud_compute_instance" ]] ; then
        # TODO: use network specific as well as predeployed nodes prep be fixed
        # CONTROLLER_NODES="$(get_openstack_node_names $openstack_node controller internalapi)"
