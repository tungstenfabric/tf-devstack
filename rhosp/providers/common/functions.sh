...
function collect_deployment_log() {
    ...
    local hosts="$host_name $(get_servers_ips)"
    for host in $hosts; do
        scp $ssh_opts $my_dir/../common/collect_logs.sh $SSH_USER@$host:
        ssh $ssh_opts $SSH_USER@$host TF_LOG_DIR="/home/$SSH_USER/logs" ./collect_logs.sh create_log_dir
        ssh $ssh_opts $SSH_USER@$host TF_LOG_DIR="/home/$SSH_USER/logs" ./collect_logs.sh collect_docker_logs
        ssh $ssh_opts $SSH_USER@$host TF_LOG_DIR="/home/$SSH_USER/logs" ./collect_logs.sh collect_system_stats
        ssh $ssh_opts $SSH_USER@$host TF_LOG_DIR="/home/$SSH_USER/logs" ./collect_logs.sh collect_contrail_logs
        source_name=$(ssh $ssh_opts $SSH_USER@$host hostname -s)
        mkdir ${TF_LOG_DIR}/${host_name}
        scp -r $ssh_opts $SSH_USER@$host:logs/* ${TF_LOG_DIR}/${source_name}/
    done
