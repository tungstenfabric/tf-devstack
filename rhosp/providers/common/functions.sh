...
function get_server_internal_ip() {
    local node_ip=$1
    local vlan_id=$2
...
function collect_overcloud_env() {
    if [[ "$DEPLOY_COMPACT_AIO" == "true" ]] ; then
        ...
        for node in $CONTROLLER_NODES_PROV_IPS ; do
            CONTROLLER_NODES="$(get_server_internal_ip $node $internal_vlan) $CONTROLLER_NODES"
        done
        for node in $AGENT_NODES_PROV_IPS ; do
            AGENT_NODES="$(get_server_internal_ip $node $internal_vlan) $AGENT_NODES"
        done
    else
...
function add_vlan_interface() {
    ...
    local ip_addr=$3
    local net_mask=$4
cat << EOM > /etc/sysconfig/network-scripts/ifcfg-${vlan_id}
