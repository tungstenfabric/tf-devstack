...
function collect_overcloud_env() {
    if [[ "${DEPLOY_COMPACT_AIO,,}" == 'true' ]] ; then
        ...
        DEPLOYMENT_ENV['DPDK_AGENT_NODES']=$(get_openstack_node_ips contraildpdk tenant)
        DEPLOYMENT_ENV['SRIOV_AGENT_NODES']=$(get_openstack_node_ips contrailsriov tenant)
        AGENT_NODES="$(get_openstack_node_ips novacompute tenant) ${DEPLOYMENT_ENV['DPDK_AGENT_NODES']} ${DEPLOYMENT_ENV['SRIOV_AGENT_NODES']}"
        ...
        DEPLOYMENT_ENV['SRIOV_AGENT_NODES']=$(get_servers_ips_by_name contrailsriov)
        AGENT_NODES="$(get_servers_ips_by_name novacompute) ${DEPLOYMENT_ENV['DPDK_AGENT_NODES']} ${DEPLOYMENT_ENV['SRIOV_AGENT_NODES']}"
    ...
    DEPLOYMENT_ENV['SSH_USER_OVERCLOUD']="$SSH_USER_OVERCLOUD"
    DEPLOYMENT_ENV['SSH_USER']="$SSH_USER"
}
