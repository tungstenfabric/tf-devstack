...
function get_openstack_node_names() {
    ...
    if [[ "${USE_PREDEPLOYED_NODES,,}" == true ]]; then
        ...
        [[ "$name" == 'controller' ]] &&  printf "%s.$suffix " ${overcloud_cont_instance//,/ } && return
        [[ "$name" == 'contrailcontroller' ]] && printf "%s.$suffix " ${overcloud_ctrlcont_instance//,/ } && return
        [[ "$name" == 'novacompute' ]] && printf "%s.$suffix " ${overcloud_compute_instance//,/ } && return
        echo "ERROR: unsupported node role $name"
        exit 1;
    fi
    ssh $ssh_opts $SSH_USER_OVERCLOUD@$openstack_node \
...
function collect_overcloud_env() {
    ...
    CONTROLLER_NODES="$(get_openstack_nodes contrailcontroller internalapi)"
