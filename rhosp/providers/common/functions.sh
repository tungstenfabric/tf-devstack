...
function get_openstack_node_ips() {
    local net=$1
    [[ -z "$OS_AUTH_URL" ]] && source ~/stackrc
    openstack port list --network $net -c Name -c "Fixed IP Addresses" -f value | \
                        grep -v virtual | grep -o '[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*' | xargs
}
...
function collect_overcloud_env() {
    if [[ "${DEPLOY_COMPACT_AIO,,}" == 'true' ]] ; then
        ...
        DEPLOYMENT_ENV['OPENSTACK_CONTROLLER_NODES']=$(get_openstack_node_ips external)
    else
