...
function collect_overcloud_env() {
    ...
    if [[ "$DEPLOY_COMPACT_AIO" == "true" ]] ; then
        ...
        for node in $controller_nodes_prov_ips ; do
            CONTROLLER_NODES="$(get_server_vlan_ips $node $internal_vlan) $CONTROLLER_NODES"
        done
        for node in $agent_nodes_prov_ips ; do
            AGENT_NODES="$(get_server_vlan_ips $node $internal_vlan) $AGENT_NODES"
        done
    else
