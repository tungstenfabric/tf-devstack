...
function get_server_interface_ips() {
    local server_ip=$1
    local server_iface=$2
    ssh $server_ip "PATH=\$PATH:/sbin; ip addr show $server_iface | \
...
function collect_overcloud_env() {
    ...
    local internal_vip=$(ssh $ssh_opts $SSH_USER@$openstack_node sudo hiera -c /etc/puppet/hiera.yaml internal_api_virtual_ip)
    if [[ "$DEPLOY_COMPACT_AIO" == "true" ]] ; then
        ...
        for node in $controller_nodes_prov_ips ; do
            CONTROLLER_NODES="$(get_server_interface_ips $node $internal_vlan) $CONTROLLER_NODES"
        done
        for node in $agent_nodes_prov_ips ; do
            AGENT_NODES="$(get_server_interface_ips $node $internal_vlan) $AGENT_NODES"
        done
        [[ -z "$OS_AUTH_URL" ]] && source ~/stackrc
        OPENSTACK_CONTROLLER_NODES="$(openstack port list --network external -c Name -c "Fixed IP Addresses" -f value | \
                                                grep -v public | grep -o '[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*' | xargs)"
    else
    ...
    if [[ -f ~/overcloudrc ]] ; then
        source ~/overcloudrc
        local os_auth_internal_api_url=$(echo $OS_AUTH_URL | sed "s#[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*\+#$internal_vip#")
    ...
    if [[ "$ENABLE_NETWORK_ISOLATION" = true ]] ; then
        DEPLOYMENT_ENV['OPENSTACK_CONTROLLER_NODES']="${OPENSTACK_CONTROLLER_NODES}"
    fi
}
