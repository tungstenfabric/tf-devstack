...
EOF
        ...
        rsync -a --safe-links -e "ssh -i $ssh_opts" $SSH_USER_OVERCLOUD@$ip:logs/ ${TF_LOG_DIR}/${source_name}/
    done
    if [[ "$ENABLE_TLS" == 'ipa' ]] ; then
...
EOF
        ...
        rsync -a --safe-links -e "ssh -i $ssh_opts" $SSH_USER@$ip:logs/ ${TF_LOG_DIR}/ipa/
    fi
