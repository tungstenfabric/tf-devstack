...
rsync -a -e "ssh -i $ssh_private_key $ssh_opts" rhosp-environment-bmc.sh $tf_dir $SSH_USER@$instance_ip:rhosp-environment.sh
if [[ -n "$ENABLE_TLS" ]] ; then
  wait_ssh ${ipa_mgmt_ip} ${ssh_private_key}
  rsync -a -e "ssh -i $ssh_private_key $ssh_opts" $tf_dir root@${ipa_mgmt_ip}:
  rsync -a -e "ssh -i $ssh_private_key $ssh_opts" rhosp-environment-bmc.sh $tf_dir root@${ipa_mgmt_ip}:rhosp-environment-bmc.sh
fi
