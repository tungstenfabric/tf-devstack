...
rsync -a -e "ssh -i $ssh_private_key $ssh_opts" rhosp-environment.sh instackenv.json $tf_dir $SSH_USER@$instance_ip:
ssh $ssh_opts -i $ssh_private_key $SSH_USER@${instance_ip} sed -i 's/PROVIDER=.*/PROVIDER=bmc/g' rhosp-environment.sh
if [[ -n "$ENABLE_TLS" ]] ; then
  ...
  ssh $ssh_opts -i $ssh_private_key root@${ipa_mgmt_ip} sed -i 's/PROVIDER=.*/PROVIDER=bmc/g' rhosp-environment.sh
fi
