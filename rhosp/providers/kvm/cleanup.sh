...
ssh -T $ssh_opts $SSH_USER@${instance_ip} "ENABLE_RHEL_REGISTRATION=$ENABLE_RHEL_REGISTRATION ./tf-devstack/rhosp/providers/common/cleanup.sh"

if [[ "$ENABLE_TLS" == 'ipa' ]] ; then
    ssh -T $ssh_opts $SSH_USER@${ipa_mgmt_ip} "sudo subscription-manager unregister" || true
fi
