sudo -E bash -c "CONTAINER_REGISTRY='' DEPLOYER_CONTAINER_REGISTRY='' CONFIGURE_DOCKER_LIVERESTORE=false ${my_dir}/../../common/create_docker_config.sh"
...
if [[ -n "$DEPLOYER_CONTAINER_REGISTRY" ]] && is_registry_insecure "$DEPLOYER_CONTAINER_REGISTRY" ; then
    echo "INFO: add DEPLOYER_CONTAINER_REGISTRY=$DEPLOYER_CONTAINER_REGISTRY to insecure list"
    insecure_registries+=" --insecure-registry $DEPLOYER_CONTAINER_REGISTRY"
fi
